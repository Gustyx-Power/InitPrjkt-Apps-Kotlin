...
check_java_version() {
    ...
    if command -v java &> /dev/null; then
        ...
        if [[ "$JAVA_VERSION" =~ ^[0-9]+$ ]] && [ "$JAVA_VERSION" -ge "$REQUIRED_JAVA_VERSION" ]; then
            ...
            echo "Java 21 will be installed..."
            return 1
        fi
    else
        echo -e "${YELLOW}[INFO] Java not found. Java $REQUIRED_JAVA_VERSION will be installed...${NC}"
        return 1
    fi
}
...
install_dependencies() {
    if [[ "$OS_TYPE" == "Linux" ]]; then
        ...
        echo -e "${YELLOW}Detected Linux distribution family: $DISTRO${NC}"
        ...
        case "$DISTRO" in
            "debian")
                ...
                if ! command -v curl &> /dev/null || ! command -v wget &> /dev/null || ! command -v unzip &> /dev/null; then
                    ...
                    sudo apt update
                    sudo apt install -y curl wget unzip openjdk-21-jdk
                fi
                ;;
            "fedora")
                # Fedora/RHEL family
                if ! command -v curl &> /dev/null || ! command -v wget &> /dev/null || ! command -v unzip &> /dev/null; then
                    ...
                    if command -v dnf &> /dev/null; then
                        sudo dnf install -y curl wget unzip java-21-openjdk-devel
                    else
                        sudo yum install -y curl wget unzip java-21-openjdk-devel
                    fi
                fi
                ;;
            "arch")
                ...
                if ! command -v curl &> /dev/null || ! command -v wget &> /dev/null || ! command -v unzip &> /dev/null; then
                    ...
                    sudo pacman -S --needed curl wget unzip jdk21-openjdk
                fi
                ;;
            *)
                echo -e "${YELLOW}Unknown Linux distribution. Please install curl, wget, unzip, and Java 21 manually.${NC}"
                ;;
        esac
...
detect_sdk() {
    ...
    # Install dependencies for Linux (including Java 21 if needed)
    install_dependencies
...
install_sdk_components() {
    if [[ "$INSTALL_SDK" == "y" || "$INSTALL_SDK" == "yes" ]]; then
        ...
        if [ ! -f "$SDKMANAGER" ]; then
            ...
            CMDTOOLS_URL="https://github.com/Gustyx-Power/InitPrjkt-Apps-Kotlin/releases/download/cmd-tools/commandlinetools-linux-14742923.zip"
            CMDTOOLS_ZIP="/tmp/commandlinetools-linux.zip"
