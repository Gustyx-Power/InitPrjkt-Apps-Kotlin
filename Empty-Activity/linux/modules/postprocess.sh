...
open_project_suggestion() {
    ...
    read -p "Open project folder in file manager? [Y/n]: " OPEN_FOLDER
    ...
    if [[ "$OPEN_FOLDER" != "n" && "$OPEN_FOLDER" != "no" ]]; then
        ...
        if command -v xdg-open &> /dev/null; then
            xdg-open "$FULL_PROJECT_PATH" &
        elif command -v nautilus &> /dev/null; then
            nautilus "$FULL_PROJECT_PATH" &
        elif command -v dolphin &> /dev/null; then
            dolphin "$FULL_PROJECT_PATH" &
        elif command -v thunar &> /dev/null; then
            thunar "$FULL_PROJECT_PATH" &
        else
            echo -e "${YELLOW}Could not detect file manager. Please open manually: $FULL_PROJECT_PATH${NC}"
        fi
    fi
}